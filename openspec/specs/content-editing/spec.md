...
How a page is edited from the browser and written back to the repository: the commit-on-save write path, the addressing rule a save must satisfy, optimistic concurrency, the single per-repo write lock, and the transactional guarantee that a failed save leaves no partial state.
