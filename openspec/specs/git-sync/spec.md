...
How ZeroWiki exchanges commits with a git client: a Smart HTTP remote behind wiki authentication, accepting pushes into the checked-out branch, re-indexing and notifying open viewers when a push lands, Obsidian vault compatibility, and mapping a git identity to an account.
