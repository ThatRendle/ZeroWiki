...
- [ ] 2.5 Test the pre-existing root `.gitignore`: an unrelated rule (append, both survive); the exact rule already present as its own line (append nothing, file left byte-identical); and the rule present only inside a comment (append, because a commented line is not the rule) — the third is the one a substring check fails (D5)
- [ ] 2.6 Pin git configuration in every fixture this section adds. The rule itself no longer consults git, but the tests still assert that a path *is ignored*, and that assertion reads `$GIT_DIR/info/exclude` and `core.excludesFile` — an unpinned fixture inherits the developer's machine and can pass for a reason that has nothing to do with the code
- [ ] 2.7 Test the fail-fast path: a volume whose pre-existing ignore file matches `.gitignore` itself makes bootstrap fail rather than commit without the rule — asserting the refusal, not merely that something threw
