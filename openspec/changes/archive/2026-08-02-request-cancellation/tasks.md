...
- [x] 4.5 Sweep: no page passes a request-scoped token to a de-authorisation call, and no page omits one from a read or create. The other half of 4.3, and the **only** mechanical evidence §3's work exists — §3 changed no runtime behaviour, since the omitted argument it replaced already bound to `CancellationToken.None` via the services' `= default`
