...
- **Single per-repo write lock** (cross-process `flock`) taken by the app — around its own commit path, and around the whole `git http-backend` invocation that serves a push — so browser commits and Obsidian pushes are mutually exclusive and the working tree is never left dirty.
