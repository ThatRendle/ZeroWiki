...
/// <summary>
/// Exercises the account page's git access tokens over HTTP against the real application. The
/// property this page exists to hold — a credential rendered exactly once and unrecoverable
/// afterwards — is only observable across separate requests, which is only observable here.
/// </summary>
public sealed partial class AccountPageTests : IDisposable
{
    ...
    {
        // The §7 projection hazard, designed out rather than survived: the list is a projection and
        // the username comes off the signed-in principal, so no Account row is ever materialised
        // here and a value-converted timestamp nothing can parse cannot take this page down.
        await SeedAccountAsync("alice");
        var client = await SignInAsync("alice");
        await GenerateTokenAsync(client);
        var tokenId = Assert.Single(await GetTokensAsync()).Id;
    ...
    [Fact]
    public async Task The_page_posts_nothing_but_the_two_fields_its_forms_need()
    {
        await SeedAccountAsync("alice");
        var client = await SignInAsync("alice");
        await GenerateTokenAsync(client);
        ...
        // be absent could have named it in advance. The revoke buttons carry their identifier as a
        // <button> value, which is why "RevokeInput.TokenId" is not an input here.
        Assert.Equal(
